...
9. Now we need to create the spider that will browse the website and scrape out CatalogItems for us. For that we'll need to delve down into the `spiders` folder, which is currently empty except for a blank `__init__.py` file.
